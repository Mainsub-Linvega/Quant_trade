...
> ⚠️ **2026-08-31 结案后追记：本手册已全部执行完毕，属历史记录。**
> 文内多处 zip 清单与表格在执行过程中**被后续订正覆盖过三次**（逐段都留了订正告示）。
> 如果你只想知道「最后交出去的是哪个包」，**唯一有效的是 §D6 的「⭐ 8/31 上传日卡片」**：
> 主件 `outputs/v3_hybrid_submission_20260829.zip`（sha `d934d246…`，13 文件），
> 其余所有 zip 一律不可交。阶段编号（`D0`…`D6`）的含义见 [`GLOSSARY.md`](GLOSSARY.md) §1。
...
🛑 **2026-08-29 第三次订正：本表/本段的 zip 全部不可交** —— 缺显式 `num_threads`，会撞穿 `total_timeout` 并让 85.8% 的 `time_id` 填 0。以 §D6「⭐ 8/31 上传日卡片」为准（主件 `..._20260829.zip`，`d934d246…`）。
...
**✅ 2026-08-31 复核：`~/submit` 不是门禁（曾被误判为缺口，当天由用户向主办方问清后关闭）。**
官方 `public_release_20260823/docs/submission_and_evaluation.md` 写「评测系统会导入
`~/submit/main.py` 中的 `class Model`」，检查清单也有「`~/submit` 已创建，内容与 ZIP 一致」——
读起来像是评测真的从选手 Home 下那个目录导入代码。**不是。**
**用户 2026-08-31 向主办方确认：提交策略的存放路径与开发环境（JupyterHub）是分开的，
选手看不到。** ⟹ 文档里的 `~/submit` 只是**自检用的本地目录**（配合
`run_timeseries_api.py --strategy-dir ~/submit` 跑通过流程），评测端实际用的是上传的 zip。
本机 JupyterHub 上也实测**没有 `~/submit`** —— 而文档明写「未创建该目录的用户，主办方会在
评测前代为创建并放入压缩包内所有文件」，本身就是合法状态。
⟹ **上传前不需要对 `~/submit` 做任何事。**

⚠️ 留档这一条是因为**推断的形状是对的、结论是错的**：我按伤疤规则 17 推「代码从哪个目录被
导入」这个口径不在我们量过的东西里，于是担心残留旧目录（`ada6a2c2…` 那版无 `num_threads`
的 `main.py`）被误用。**但归属检查不能靠读文档措辞推，得去问那个系统的真值来源** ——
一句向主办方的确认就把整条假想路径消掉了。
