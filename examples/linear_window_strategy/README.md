...
```bash
python inference_api/linear_window_strategy/train.py \
  --release-root /path/to/release \
  --model-dir inference_api/linear_window_strategy/model \
  --window-size 5
```
...
python scripts/run_timeseries_api.py \
  --release-root /path/to/release \
  --strategy-dir inference_api/linear_window_strategy \
