...
**已排除的解释**：相位错配。造了 `--holdout-phase`，在模型完全没见过的相位上评估，
方向没变。**下一个嫌疑是预测时长**：本地每折验证段只有约 49,345 个原始 time_id，
公榜测试集有 214,538 个（外推远 4.35 倍）。见 ROADMAP 第 1 项。

**实际影响**：凡是影响「拟合紧密度」的改动（alpha、特征数、模型复杂度），
本地结论目前不可信；纯后处理旋钮（scale、clip）和结构性诊断不受影响。
