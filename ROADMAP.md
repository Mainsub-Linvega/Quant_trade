...
- **NumPy 兜底全量已实测：`predict_total = 10.44 分钟`**（2026-08-18，3,217,458 行 /
  214,538 次调用 / 0 超时 / max_step 0.707s / 0 非有限值 / 0 触 clip）。此前记的「约 15 分钟」
  是旧估算，现予更新。测法：用 shim 让 `import lightgbm` 抛错，走官方 runner ——
  这就是评测机上 lightgbm 不可用时的真实路径，不是拿 `--backend` 假装
  （那个参数根本传不进官方 runner）。
  输出与 lightgbm 版**全量 321 万行逐位相同**（staging 内部对拍 2.082e-16）。
  ⟹ 兜底仍是主要剩余风险（比主路径慢 2.03×），但比原估算宽裕。
