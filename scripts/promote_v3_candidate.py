...
PUBLIC_BASELINE = {
    ...
    # ⚠️ 2026-08-21：长窗块转正后，榜上那份**就是**带 512 长窗的。
    # 该键 None → 512 是公榜 0.0041833953 与前一版 0.0041150085 的**全部差别**
    # （市场森林与冻结岭回归 hash 逐字节相同，只重训了截面森林）。
    # 加进这张表的理由同 slow/fast（CLAUDE.md §8.2）：不在表里的键，
    # 丢了或写错都不会被任何门禁发现 —— 缺键时 main.py 会**静默关掉长窗**，
    # 交出去的就是低 1.66% 的旧模型。
    "long_window": 512,
}
